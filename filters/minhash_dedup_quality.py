...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Minhash deduplication pipeline for quality-classified JSONL data using datatrove."
    )

    # Required arguments
    parser.add_argument("--input-dir", required=True,
                        help="Path to the quality-classified JSONL input directory.")
    ...
    # Quality label
    parser.add_argument("--quality-label", default="high", choices=["low", "medium", "high"],
                        help="Quality label subdirectory to process (default: high).")

    # Executor arguments
    parser.add_argument("--job-name", default="mhq-dedup",
                        help="Base name for executor jobs (default: mhq-dedup).")
    ...
    slurm.add_argument("--time-limit", default=None,
                       help="SLURM time limit, e.g. '18:00:00'.")
...
if __name__ == '__main__':
    ...
    input_reader = JsonlReader(
        data_folder=f"{args.input_dir}/{label}",
    ...
    minhash_config = MinhashConfig(
        ...
        num_buckets=40,
        hashes_per_bucket=15,
        n_grams=4,
    )

    # Stage 1: Compute minhash signatures
    stage1 = MareNostrumExecutor(
        job_name=f"{args.job_name}-1sig-{label}",
        pipeline=[
            input_reader,
            MinhashDedupSignature(
                output_folder=f"{args.output_dir}/{label}/steps/1_signatures",
        ...
        logging_dir=f"{args.logging_dir}/{label}/1_signatures",
        slurm_logs_folder=f"{args.logging_dir}/{label}/1_signatures/slurm_logs",
    ...
    stage2 = MareNostrumExecutor(
        job_name=f"{args.job_name}-2bkt-{label}",
        pipeline=[
            MinhashDedupBuckets(
                input_folder=f"{args.output_dir}/{label}/steps/1_signatures",
                output_folder=f"{args.output_dir}/{label}/steps/2_buckets",
                config=minhash_config,
            ),
        ],
        ...
        cpus_per_task=args.cpus_per_task,
        randomize_start_duration=args.randomize_start_duration,
        logging_dir=f"{args.logging_dir}/{label}/2_buckets",
        slurm_logs_folder=f"{args.logging_dir}/{label}/2_buckets/slurm_logs",
        depends=stage1,
        **slurm_kwargs,
    )
    ...
    stage3 = MareNostrumExecutor(
        job_name=f"{args.job_name}-3cls-{label}",
        pipeline=[
            MinhashDedupCluster(
                input_folder=f"{args.output_dir}/{label}/steps/2_buckets",
                output_folder=f"{args.output_dir}/{label}/steps/3_clusters",
                config=minhash_config,
        ...
        tasks=1,
        cpus_per_task=args.cpus_per_task,
        logging_dir=f"{args.logging_dir}/{label}/3_clusters",
        slurm_logs_folder=f"{args.logging_dir}/{label}/3_clusters/slurm_logs",
        depends=stage2,
        **slurm_kwargs,
    )
    ...
    stage4 = MareNostrumExecutor(
        job_name=f"{args.job_name}-4flt-{label}",
        pipeline=[
            input_reader,
            MinhashDedupFilter(
                input_folder=f"{args.output_dir}/{label}/steps/3_clusters",
                exclusion_writer=JsonlWriter(f"{args.output_dir}/{label}/removed"),
                load_cluster_ids=True,
                load_cluster_sizes=True
            ),
            Rehydrater(),
            TokensCounter(),
            JsonlWriter(output_folder=f"{args.output_dir}/{label}/output"),
        ],
        tasks=args.tasks,
        workers=args.workers,
        cpus_per_task=args.cpus_per_task,
        logging_dir=f"{args.logging_dir}/{label}/final_output",
        slurm_logs_folder=f"{args.logging_dir}/{label}/final_output/slurm_logs",
        depends=stage3,
        **slurm_kwargs,
    )
